...
class BWFAN_API_Carts_View_Tasks extends BWFAN_API_Base {
	...
	public function process_api_call() {
		...
		if ( empty( $abandoned_id ) ) {
			return $this->error_response( __( 'Abandoned id is missing.', 'wp-marketing-automations' ) );
		}

		$get_cart_tasks = BWFAN_Recoverable_Carts::get_cart_tasks( $abandoned_id );
		if ( empty( $get_cart_tasks ) ) {
			return $this->success_response( $get_cart_tasks, __( 'No Tasks Details Available for abandoned id:.' . $abandoned_id, 'wp-marketing-automations' ) );
		}

		return $this->success_response( $get_cart_tasks, __( 'Cart Tasks Details.', 'wp-marketing-automations' ) );
	}
}
