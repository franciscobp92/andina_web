	...
	public static function test_sms() {
		...
		if ( bwfan_is_autonami_pro_active() && class_exists( 'BWFAN_UTM_Tracking' ) ) {
			...
			if ( version_compare( BWFAN_PRO_VERSION, '2.0.4', '>' ) ) {
				$data_to_set['sms_body'] = $utm->maybe_add_utm_parameters( $data_to_set['sms_body'], $data_to_set, 'sms' );
			} else {
				$data_to_set['sms_body'] = $utm->maybe_add_utm_parameters( $data_to_set['sms_body'], $data_to_set );
			}
		}
