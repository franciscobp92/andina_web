...
class BWFAN_Common {
	...
	/**
	 * Get the default shortener provider
	 *
	 * @return false|mixed|void
	 */
	...
	public static function run_as_ct_worker() {
		...
		if ( false === BWFAN_Model_Tasks::maybe_tasks_available() ) {
			return;
		}
	...
	public static function save_marketing_status_for_user( $contact, $order_id ) {
		...
		/** added filter to allow double optin and can still make the contact unverfied in case of consent is checked */
		if ( ! apply_filters( 'bwfcrm_contact_enable_single_optin', true, $contact, $order, $marketing_status ) ) {
			$contact_creation_date = get_date_from_gmt( $contact->get_creation_date() );
			$joined_date           = strtotime( $contact_creation_date );
			$current_time          = current_time( 'timestamp' );
			$contact_joined_diff   = $current_time - $joined_date;
			$contact_joined_diff   = intval( $contact_joined_diff / MINUTE_IN_HOURS );

			/** if the creation time is less than an hour then mark contact as unverified */
			if ( $contact_joined_diff < 1 ) {
	...
	public static function get_lists_preference_schema() {
		...
		return array(
			...
			array(
				...
				'label'    => __( 'Public Lists', 'wp-marketing-automations' ),
				'type'     => 'checkbox_grid',
			...
			array(
				...
				'label'         => __( 'Lists Visibility', 'wp-marketing-automations' ),
				'type'          => 'checkbox',
				'checkboxlabel' => __( 'Show contact their subscribed Public Lists', 'wp-marketing-automations' ),
				'hint'          => __( 'If unchecked all Public Lists will be shown to contacts', 'wp-marketing-automations' ),
	...
	public static function get_carts_count() {
		...
		return [
			'recoverable' => absint( $recoverable_count['total_count'] ) > 0 ? absint( $recoverable_count['total_count'] ) : 0,
			'recovered'   => absint( $recovered_count['total_count'] ) > 0 ? absint( $recovered_count['total_count'] ) : 0,
			'lost'        => absint( $lost_carts['total_count'] ) > 0 ? absint( $lost_carts['total_count'] ) : 0,
		];
	}
