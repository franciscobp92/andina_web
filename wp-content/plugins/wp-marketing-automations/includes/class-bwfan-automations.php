...
class BWFAN_Automations {
	...
	public function get_active_v1_automation_names() {
		...
		$query = "SELECT a.ID, a.event, meta.meta_value FROM `{$table_automation}` as `a` INNER JOIN `{$table_automation_meta}` as `meta` ON meta.bwfan_automation_id = a.ID WHERE a.status = 1 AND meta.meta_key = 'title'";
