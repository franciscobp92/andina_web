...
class BWFAN_DB {
	...
	public function db_update_2_0_7( $version_key ) {
		...
		/** Add primary key in bwfan_contact_automations table */
		$query = "ALTER TABLE `{$wpdb->prefix}bwfan_contact_automations` ADD `ID` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`ID`);";
		$wpdb->query( $query );

		/** Add Indexes on bwfan_tasks table */
		$wpdb->query( "ALTER TABLE `{$wpdb->prefix}bwfan_tasks` ADD KEY e_date(e_date), ADD KEY claim_id(claim_id);" );
