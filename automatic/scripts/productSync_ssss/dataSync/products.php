<?php
    ...
    function insertProduct($product,$conn)
    {
        ...
        {
            // $sql ="UPDATE al_posts SET post_title='".$product["PRO_NOMBRE"]."', post_content='".$desc."', post_excerpt='".$descp."' WHERE ID=".$product["PRO_CODIGO"].";";
    ...
    {
        $bandera=true;
        $sql="SELECT id from al_posts where id='".$idProduct."' and post_type='product'";
        $resultado=mysqli_query($conn, $sql) or die (mysql_error());
        if (mysqli_num_rows($resultado)==0)
        {
            $bandera= false;
